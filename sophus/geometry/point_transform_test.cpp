  ...
  for (const InverseDepthPoint3F64& inv_dept_point :
    ...
    Eigen::Matrix<double, 2, 3> const num_dx = vectorFieldNumDiff<double, 2, 3>(
        ...
        Eigen::Vector3d(
            ...
            inv_dept_point.psi()));
    FARM_CHECK_NEAR(dx, num_dx, kEpsilonF64);
  ...
  for (const InverseDepthPoint3F64& point :
    ...
    Eigen::Matrix<double, 2, 6> const num_dx2 =
        vectorFieldNumDiff<double, 2, 6>(
            ...
            zero);
    FARM_CHECK_NEAR(num_dx2, num_dx, kEpsilonF64);
  }
}
...
TEST(point_transform, integrations) {
  ...
  for (const Eigen::Vector<double, 6>& t : tangent_vec) {
    ...
    for (const auto& point_in_bar : point_vec) {
      ...
      FARM_CHECK_NEAR(xy1_by_z_in_foo, xy1_by_z_in_foo2, eps);
      FARM_CHECK_NEAR(xy1_by_z_in_foo, xy1_by_z_in_foo3, eps);
      FARM_CHECK_NEAR(xy1_by_z_in_foo, xy1_by_z_in_foo4, eps);
      ...
      Eigen::Matrix<double, 2, 6> const num_dx2 =
          vectorFieldNumDiff<double, 2, 6>(
              ...
              zero);
      FARM_CHECK_NEAR(dx, num_dx, 10 * kEpsilonSqrtF64);
      FARM_CHECK_NEAR(num_dx, num_dx2, kEpsilonSqrtF64);
      {
        ...
        FARM_CHECK_NEAR(dx, num_dx, 0.002);
      }
    }
    ...
    for (const auto& point_in_bar : point_vec) {
      ...
      Eigen::Matrix<double, 2, 6> const num_dx2 =
          vectorFieldNumDiff<double, 2, 6>(
              ...
              zero);
      FARM_CHECK_NEAR(dx, num_dx2, kEpsilonSqrtF64);
    }
  }
}
