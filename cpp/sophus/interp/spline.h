...
class SplineBasisFunction {
 public:
  static SOPHUS_FUNC Eigen::Matrix<ScalarT, 3, 4> c() {
    Eigen::Matrix<ScalarT, 3, 4> c;
    ScalarT const o(0);

    // clang-format off
    c << ScalarT(5./6),  ScalarT(3./6), -ScalarT(3./6),  ScalarT(1./6),
         ScalarT(1./6),  ScalarT(3./6),  ScalarT(3./6), -ScalarT(2./6),
                    o,             o,             o,  ScalarT(1./6);
    // clang-format on
    return c;
  }

  static SOPHUS_FUNC Eigen::Vector3<ScalarT> b(ScalarT const& u) {
    // FARM_CHECK(u >= ScalarT(0), "but %", u);
    // FARM_CHECK(u < ScalarT(1), "but %", u);
    ScalarT u_square(u * u);
    return c() * Eigen::Vector4<ScalarT>(ScalarT(1), u, u_square, u * u_square);
  }

  static SOPHUS_FUNC Eigen::Vector3<ScalarT> dtB(
      ScalarT const& u, ScalarT const& delta_t) {
    // FARM_CHECK(u >= ScalarT(0), "but %", u);
    // FARM_CHECK(u < ScalarT(1), "but %", u);
    return (ScalarT(1) / delta_t) * c() *
           Eigen::Vector4<ScalarT>(
               ScalarT(0), ScalarT(1), ScalarT(2) * u, ScalarT(3) * u * u);
  }

  static SOPHUS_FUNC Eigen::Vector3<ScalarT> dt2B(
      ScalarT const& u, ScalarT const& delta_t) {
    // FARM_CHECK(u >= ScalarT(0), "but %", u);
    // FARM_CHECK(u < ScalarT(1), "but %", u);
    return (ScalarT(1) / (delta_t * delta_t)) * c() *
           Eigen::Vector4<ScalarT>(
               ScalarT(0), ScalarT(0), ScalarT(2), ScalarT(6) * u);
  }
};
...
class BasisSplineFn {
 public:
  ...
  using ScalarT = typename LieGroup::Scalar;
  ...
  static Transformation dt2ParentPoseSpline(
    ...
    return parent_ts_control_point.matrix() *
           ((std::get<0>(dt2_aa) * std::get<1>(aa).matrix() *
            ...
            (std::get<0>(aa).matrix() * std::get<1>(aa).matrix() *
             std::get<2>(dt2_aa)) +
            ScalarT(2) * ((std::get<0>(dt_aa) * std::get<1>(dt_aa) *
                           std::get<2>(aa).matrix()) +
                          (std::get<0>(dt_aa) * std::get<1>(aa).matrix() *
                           std::get<2>(dt_aa)) +
                          (std::get<0>(aa).matrix() * std::get<1>(dt_aa) *
...
struct BasisSplineSegment {
 public:
  using T = typename LieGroupT::Scalar;
  using Transformation = typename LieGroupT::Transformation;
  ...
  [[nodiscard]] Eigen::Map<LieGroupT const> worldPoseFooPrev() const {
    return Eigen::Map<LieGroupT const>(raw_params0_);
  }
  [[nodiscard]] Eigen::Map<LieGroupT const> worldPoseFoo0() const {
    return Eigen::Map<LieGroupT const>(raw_params1_);
  }

  [[nodiscard]] Eigen::Map<LieGroupT const> worldPoseFoo1() const {
    return Eigen::Map<LieGroupT const>(raw_params2_);
  }

  [[nodiscard]] Eigen::Map<LieGroupT const> worldPoseFoo2() const {
    return Eigen::Map<LieGroupT const>(raw_params3_);
  }
  LieGroupT parentPoseSpline(double u) {
    switch (segment_case_) {
      case SegmentCase::first:
        return BasisSplineFn<LieGroupT>::parentPoseSpline(
            worldPoseFoo0(),
            std::make_tuple(
  ...
  Transformation dtParentPoseSpline(double u, double delta_t) {
    switch (segment_case_) {
      case SegmentCase::first:
        return BasisSplineFn<LieGroupT>::dtParentPoseSpline(
            worldPoseFoo0(),
            std::make_tuple(
                (worldPoseFoo0().inverse() * worldPoseFoo0()).log(),
                (worldPoseFoo0().inverse() * worldPoseFoo1()).log(),
  ...
  Transformation dt2ParentPoseSpline(double u, double delta_t) {
    switch (segment_case_) {
      case SegmentCase::first:
        return BasisSplineFn<LieGroupT>::dt2ParentPoseSpline(
            worldPoseFoo0(),
            std::make_tuple(
                (worldPoseFoo0().inverse() * worldPoseFoo0()).log(),
                (worldPoseFoo0().inverse() * worldPoseFoo1()).log(),
...
class BasisSplineImpl {
 public:
  ...
  BasisSplineImpl(
      const std::vector<LieGroup>& parent_ts_control_point, double delta_t)
  ...
  [[nodiscard]] const std::vector<LieGroup>& parentPosesControlPoint() const {
