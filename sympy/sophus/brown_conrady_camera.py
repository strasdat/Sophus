...
from sophus.matrix import vector5
...
class BrownConradyCamera:
    ...
    def pixel_from_z1_plane(self, point_in_camera_z1_plane):
        ...
        x = x * cdist * icdist2 + self.distortion[2] * a1 + self.distortion[3] * a2
        y = y * cdist * icdist2 + self.distortion[2] * a3 + self.distortion[3] * a1

        return vector2(
            self.focal_length[0] * x + self.center[0],
            self.focal_length[1] * y + self.center[1],
        )
...
class TestBrownConrady(unittest.TestCase):
    ...
    def setUp(self):
        ...
        d0, d1, d2, d3, d4 = sympy.symbols("d[0] d[1] d[2] d[3] d[4]")
        self.dist = vector5(d0, d1, d2, d3, d4)
