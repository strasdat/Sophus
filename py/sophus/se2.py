...
class Se2:
    ...
    def log(self):
        ...
        V_inv = sophus.Matrix([[a, halftheta],
                               [-halftheta, a]])
    ...
    def hat(v):
        ...
        return sophus.So2.hat(theta).\
            row_join(upsilon).\
            col_join(sophus.Matrix.zeros(1, 3))

    def matrix(self):
        """ returns matrix representation """
        R = self.so2.matrix()
        return (R.row_join(self.t)).col_join(sophus.Matrix(1, 3, [0, 0, 1]))
    ...
    def __mul__(self, right):
        ...
        if isinstance(right, sophus.Vector2):
            ...
            return So2(self.so2 * right.so2, self.t + self.so2 * right.t)
    ...
    def Dxi_x_matrix(x, i):
        if i < 2:
            return sophus.So2.Dxi_x_matrix(x, i).\
                row_join(sophus.Matrix.zeros(2, 1)).\
                col_join(sophus.Matrix.zeros(1, 3))
        M = sophus.Matrix.zeros(3, 3)
        M[i - 2, 2] = 1
        return M
...
class TestSe2(unittest.TestCase):
    def setUp(self):
        upsilon0, upsilon1, theta = sympy.symbols(
            ...
            real=True)
        x, y = sympy.symbols('x z', real=True)
